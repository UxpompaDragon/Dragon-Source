...
namespace wServer.realm.commands
{
    internal class SendCommand : Command
    {
        ...
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
            ...
            if (!int.TryParse(args[1], out ammount))
            {
                player.SendError("Usage: /send <player name> <ammount> <currency type>");
                return false;
            }
            ...
            if (p != null)
            {
                ...
                {
                    case "gold":
                        db.UpdateCredit(p.Client.Account, ammount);
                        break;
                    case "fame":
                        db.UpdateFame(p.Client.Account, ammount);
                        break;
                    case "tokens":
                        db.UpdateFortuneToken(p.Client.Account, ammount);
                        break;
                    default:
                        player.SendError("Invalid currency type!");
                        return false;
                }
                p.SendInfo(player.Name + " has sent you " + ammount + " " + args[2].ToLower() + "!");
                player.SendInfo("Sent " + p.Name + " " + ammount + " " + args[2].ToLower() + "!");
                return true;
            }
    ...
    internal class addFameCommand : Command
    {
        ...
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
            try
            {
                if (args.Length == 0)
                {
                    player.SendHelp("Use /addFame <ammount>");
                    return false;
                }
                if (args.Length == 1)
                {
                    player.Fame += int.Parse(args[0]);
                    player.UpdateCount++;
    ...
    internal class SpawnCommand : Command
    {
        ...
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
            ...
            if (args.Length > 0 && int.TryParse(args[0], out num)) //multi
            {
                ...
                if (!(player.Client.Account.Rank > 1) && c > 200)
                {
